...
class ChatListComponent extends React.Component {
  render() {
    ...
    if(this.props.chats.length > 0){
          ...
          <List>
            {
              this.props.chats.map((_chat, _index) => {
                ...
                return(
                  <div key={_index} >
                  <ListItem onClick={() => this.selectChat(_index)} className={classes.litItem} selected={this.props.selectedChatIndex === _index } alignItems='flex-start'>
                    <ListItemAvatar>
      ...
      return(
      <main className={classes.root}>
        <Button variant='contained' fullWidth onClick={this.newChat} color='primary' className={classes.newChatBtn}>New Message</Button>
        <List></List>
      </main>
      )
    }
